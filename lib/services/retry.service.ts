...
export type ManyChatFlowPayload = { subscriberId: string; flowNs: string; fields?: { fieldId: number; value: string }[] };
...
async function runJob(job: IntegrationJob): Promise<void> {
  switch (job.type) {
    ...
    case "MANYCHAT_FLOW": {
      ...
      // El flow del asesor usa Custom Fields del propio asesor. Si este job
      // viene de una notificación fallida, reescribir primero los datos del
      // lead actual evita que el retry mande el lead anterior.
